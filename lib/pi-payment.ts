...
export const checkIncompletePayments = async (
  ...
  // Non-fatal — must never block or throw; the Pi Developer Portal checker
  // does not have a live backend so we guard with an AbortController timeout.
  try {
    ...
    // Non-fatal — do not block authentication flow
  }
};
